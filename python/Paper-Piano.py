...
# Name: Norman Cook
# Date: 10/28/2019
# Description: A piano made using a Raspberry Pi combined with
#   basic circuit elements. The user presses one of three switches 
#   to activate a note. For this, the Middle C note is played with
#   three different waveforms: square, triangle, and sawtooth.
###################################################################
...
class Note(pygame.mixer.Sound):
    # bnote that volume ranges from 0.0 to 1.0
    ...
    def build_samples(self):
        ...
        # generate samples for a square waveform
        ...
        # generate samples for a triangle waveform
        elif (self.waveform == "triangle"):
            for t in range(period):
                if ( t < period / 2):
                    if (t < period / 4):
                        samples[t] = t * (period / 42)
                    else:
                        samples[t] = ((period / 2) - t) * (period / 42)
                else:
                    if (t < period * 3 / 4):
                        samples[t] = ((period / 2) - t) * (period / 42)
                    else:
                        samples[t] = (t - period) * (period / 42)
            return samples

        # generate samples for a sawtooth waveform
...
# preset mixer initialization arguments: frequency (44.1 K), size
...
# setup the pin, frequency, and waveforms for the notes (square, triangle, sawtooth)
keys = [ 20, 16, 12 ]
freqs = [ 261.6, 261.6, 261.6 ]
...
for n in range(len(freqs)):
    notes.append(Note(freqs[n], 1, waveforms[n]))
...
try:
    while (True):
        ...
        # if the play burron was pressed
        elif (key == "play"):
            # if recording, stop
            if (recording):
                recording = False
